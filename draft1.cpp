...
void Monthly_Analysis(AccRec as[],int n){
  ...
  cout<<"              total spent is " << total<<endl<<endl;
  cout<<"The distribution of monthly income: "<<endl;
  for(int i=0;i<3;i++){
    for(int k=0;k<n;k++){
      if(list2[i]==as[k].category){
        temp=as[i].amount;
        number=atof(temp.c_str());
    ...
    for(int i=0;i<4;i++){
      cout<<list2[i]<<": "<<(price_type_list[i]/income)*100<<"% of income"<<endl;
    }
    cout<<"The distribution of monthly expense: "<<endl;
    for(int i=0;i<8;i++){
      for(int k=0;k<n;k++){
        if(list[i]==as[k].category){
          temp=as[i].amount;
          number=atof(temp.c_str());
          price_type_list[i]=price_type_list[i]+number;
        }
    ...
    for(int i=0;i<8;i++){
      sum=sum+price_type_list[i];
    }
    price_type_list[9]=expense-(sum);
    for(int i=0;i<9;i++){
      cout<<list[i]<<": "<<(price_type_list[i]/expense)*100<<"% of expense"<<endl;
    }
}
